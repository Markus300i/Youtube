...
ACTION_META: dict[str, dict[str, Any]] = {
    "tts": {"label": "TTS", "detail": "Chatterbox narration", "check": "tts", "requires": ()},
    "captions": {"label": "Captions", "detail": "Whisper subtitles", "check": "captions", "requires": ("tts",)},
    "sound_design": {"label": "Sound", "detail": "Final audio mix", "check": "sound_design", "requires": ("tts",)},
    "visual_qa": {"label": "Visual QA", "detail": "NVIDIA visual review", "check": "visual_qa", "requires": ("active_images",)},
    "opencut_export": {
        "label": "OpenCut",
        ...
        with tempfile.TemporaryDirectory(prefix="csp-studio-quick-") as tmp:
            ...
            with StudioStore(DB_PATH) as store:
                ...
                invalidate_after_image_change(
                    ...
                    reason=f"scene {task.scene_id} quick regenerated by task {task_id}",
                )
                result = {
                    "asset": asset.to_dict(),
                    "mode": "quick",
