...
class StudioTaskRunner:
    ...
    def _execute(self, task_id: str, log_path: Path) -> dict[str, Any]:
        ...
        with StudioStore(self.db_path) as store:
            TaskEngine(store).progress(task_id, 95, stage="finalize")
        return {"returncode": returncode, "log_path": str(log_path), "command": self._display_command(command)}
    ...
    def _regenerate_scene(
        ...
        with tempfile.TemporaryDirectory(prefix="csp-studio-regen-") as tmp:
            ...
            with StudioStore(self.db_path) as store:
                ...
                TaskEngine(store).progress(task_id, 85, stage="activate_revision")
                ...
                tasks.set_checkpoint(project_id, "visual_qa", "stale", metadata={"reason": f"scene {scene_id} regenerated"})
                tasks.set_checkpoint(project_id, f"visual_qa_scene_{scene_id:02d}", "stale", metadata={"reason": "image revision changed"})
                return {
