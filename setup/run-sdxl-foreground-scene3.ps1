...
Write-Host "CHECK rembg foreground tool" -ForegroundColor Cyan
$oldErrorAction = $ErrorActionPreference
$ErrorActionPreference = "Continue"
& $CspPython -c "import rembg" 2>$null
$rembgExit = $LASTEXITCODE
$ErrorActionPreference = $oldErrorAction
if ($rembgExit -ne 0) {
    Write-Host "INSTALL rembg[cpu]" -ForegroundColor Cyan
    & $CspPython -m pip install "rembg[cpu]"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to install rembg[cpu]"
    }
}
else {
    Write-Host "OK   rembg" -ForegroundColor Green
}
...
$testScript = Join-Path $RepoRoot "scripts\test_sdxl_foreground_scene3.py"
$shortFile = Join-Path $RepoRoot "shorts\001-drzwi-0.yaml"
Assert-Exists $testScript "SDXL foreground test script"
Assert-Exists $shortFile "Drzwi 0 YAML"

$logDir = Join-Path $RepoRoot "logs"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$logFile = Join-Path $logDir "sdxl-foreground-scene3.log"

Push-Location $RepoRoot
try {
    Write-Host "RUN  SDXL foreground scene 3 G/H/I" -ForegroundColor Cyan
    $oldErrorAction = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    ...
    if ($exitCode -ne 0) {
        ...
        throw "SDXL foreground test failed with exit code $exitCode. See $logFile"
    }
}
finally {
    Pop-Location
...
$result = "C:\CSP\output\001-drzwi-0\compare\scene-03\sdxl-foreground-ghi\comparison-GHI.jpg"
Write-Host ""
